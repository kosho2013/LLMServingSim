...
#[context_macro]
pub struct router_torus<A: Clone> {
...
impl<A: DAMType + num::Num> router_torus<A>
where
router_torus<A>: Context,
{
    pub fn new(
        ...
        for i in 0..in_len
        {
            ...
            router_torus.in_stream[idx].attach_receiver(&router_torus);
        }
        for i in 0..out_len
        {
            let idx: usize = i.try_into().unwrap();
            router_torus.out_stream[idx].attach_sender(&router_torus);
        }
...
impl<A: DAMType + num::Num> Context for router_torus<A> {
    fn run(&mut self) {
        ...
        loop
        {
            ...
            if lowest_idx != invalid
            {
                ...
                } else // Something
                {
                    ...
                    } else if dst_x_vec[lowest_idx] == self.x && dst_y_vec[lowest_idx] < self.y
                    {
                        let tmp1 = self.y - dst_y_vec[lowest_idx];
                        let tmp2 = self.y_dim - tmp1;
                    ...
                    {
                        let tmp1 = self.x - dst_x_vec[lowest_idx];
                        let tmp2 = self.x_dim - tmp1;
                    ...
                    {
                        let tmp1 = dst_y_vec[lowest_idx] - self.y;
                        let tmp2 = self.y_dim - tmp1;
                    ...
                    {
                        let tmp1 = dst_x_vec[lowest_idx] - self.x;
                        let tmp2 = self.x_dim - tmp1;
